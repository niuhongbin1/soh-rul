...
def make_windows(df, in_len=200, out_len=10, step=1):
    ...
    for s in range(0, len(vals)-in_len-out_len+1, step):
        ...
        X.append(seg[:in_len].T)     # (2, in_len) -> Conv1d expects (B,C,L)
...
class TCN(nn.Module):
    def __init__(self, in_ch=2, levels=6, channels=64, ks=3, dropout=0.1, out_len=10):
        ...
        for i in range(levels):
            layers.append(TBlock(ch, channels, ks, 2**i, dropout))
            ch=channels
        self.tcn = nn.Sequential(*layers)
        self.head = nn.Sequential(
            nn.AdaptiveAvgPool1d(1), nn.Flatten(),
            nn.Linear(channels, 128), nn.ReLU(),
            nn.Linear(128, out_len)
        )
    def forward(self, x):
        feat = self.tcn(x)            # (B,C,L)
        return self.head(feat)        # (B,out_len)
...
@torch.no_grad()
def rolling_forecast(model, df_norm, s_scaler, device, in_len=200, out_len=10, start_idx=70):
    ...
    while pos<len(df_norm):
        ...
        horizon = min(out_len, len(df_norm)-pos); pred[pos:pos+horizon]=out[:horizon]; pos+=horizon
...
def plot_with_error(raw_time, y_true, y_pred, title, start_idx=70):
...
def main():
    ...
    in_len,out_len,step=200,10,1; batch,epochs,lr=256,50,1e-3
    ...
    for i,(dfn,ssc) in enumerate(zip(dfs, sscs),1):
        ...
        y_true,y_pred = rolling_forecast(model, dfn, ssc, device, in_len=in_len, out_len=out_len, start_idx=70)
        plot_with_error(raw_time, y_true, y_pred, f"TCN v2 - Dataset {i} (roll from 70)", start_idx=70)
        pd.DataFrame({"time":raw_time,"state_true":y_true,"state_pred":y_pred}).to_csv(f"forecast_dataset_{i}_tcn_v2.csv", index=False)
        print(f"Saved: forecast_dataset_{i}_tcn_v2.csv")
