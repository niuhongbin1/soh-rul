...
OUTPUT_DIR = './data_parse/output_after_2nd_checkup'
NOMINAL_CAPACITY = 3.0
EFC_INTERVAL = 1.0
# ===============================================


def extract_soh(file_path, nominal_capacity, efc_interval=1.0, remove_n=2):
    """提取SOH，剔除前N次check-up"""
...
def plot_results(all_results, output_dir):
    ...
    plt.title('SOH vs EFC (剔除前2次Check-up后)', fontsize=14, fontweight='bold')
    plt.legend(loc='best', fontsize=10)
    plt.grid(True, alpha=0.3)
    plt.axhline(y=80, color='red', linestyle='--', linewidth=2, alpha=0.6)
    
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'soh_after_2nd.png'), dpi=300)
    plt.show()


def main():
    print("="*60)
    print("剔除前2次Check-up版本 - EFC重新计数")
    print("="*60)
    ...
    for file_path in FILE_PATHS:
        ...
        df = extract_soh(file_path, NOMINAL_CAPACITY, EFC_INTERVAL, remove_n=2)
